def how_many_items(arr: list[float]) -> int:
    """Find the length of an array."""
    number_of_items = 0
    while True:
        try:
            item = arr[number_of_items]
        except IndexError:
...
def remove_from_array(arr: list[float], val: float) -> list[float]:
    """Remove given value from an array."""
    length = how_many_items(arr)
    for item in range(length):
        if arr[item] == val:
            del arr[item]
...
def dumbsort(arr: list[float], choice: str) -> list[float]:
    ...
    for i in range(length1):
        ...
        length2 = how_many_items(mins)
        if length2 != 0:
            for k in range(length2):
                sub_array = remove_from_array(sub_array, mins[k])

        if how_many_items(sub_array) == 0:
            print(new_arr)
            return new_arr

        length3 = how_many_items(sub_array)
...
def main():
    ...
    a = dumbsort(array, "desc")  # choose between asc and desc
    print(a)
